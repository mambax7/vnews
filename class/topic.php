...
class vnews_topic extends XoopsObject {
    ...
    public function getForm() {
        ...
        $form = new XoopsThemeForm ( _VNEWS_AM_TOPIC_FORM, 'topic', 'backend.php', 'post' );
        ...
        global $xoopsModule;
        if (! $this->isNew ()) {
            $groups_ids_view = $gperm_handler->getGroupIds ( 'vnews_view', $this->getVar ( 'topic_id' ), $xoopsModule->getVar ( 'mid' ) );
            $groups_ids_submit = $gperm_handler->getGroupIds ( 'vnews_submit', $this->getVar ( 'topic_id' ), $xoopsModule->getVar ( 'mid' ) );
...
class VnewsTopicHandler extends XoopsPersistableObjectHandler {
     ...
     * @param $db
     */
    public function VnewsTopicHandler($db) {
        parent::XoopsPersistableObjectHandler ( $db, 'vnews_topic', 'vnews_topic', 'topic_id', 'topic_alias' );
    }
