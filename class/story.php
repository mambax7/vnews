...
class vnews_story extends XoopsObject
{
    ...
    {
        $form = new XoopsThemeForm(_VNEWS_AM_STORY_FORM, 'vnews', 'backend.php', 'post', true);
    ...
    {
        $form = new XoopsThemeForm(_VNEWS_AM_STORY_FORM, 'vnews', 'submit.php', 'post', true);
        $form->setExtra('enctype="multipart/form-data"');
...
class VnewsStoryHandler extends XoopsPersistableObjectHandler
{
    ...
    {
        parent::__construct($db, 'vnews_story', 'vnews_story', 'story_id', 'story_alias');
    }
    ...
    public function News_StoryList($story_infos)
    {
        ...
        $topicHandler = xoops_getModuleHandler('topic', 'vnews');
        $topic_show   = $topicHandler->News_TopicAllVisible($story_infos ['topics'], $story_infos ['story_topic']);
    ...
    {
        $sql = 'UPDATE ' . $this->table . ' SET story_hits = story_hits + 1 WHERE story_id= ' . (int)$story_id;
    ...
    public function News_StoryFile($action, $id, $previous = null)
    {
        switch ($action) {
            case 'add':
                $sql = 'UPDATE ' . $this->table . ' SET story_file = story_file + 1 WHERE story_id= ' . (int)$id;
                break;
    ...
    {
        $sql = 'UPDATE ' . $this->db->prefix('vnews_story') . ' SET story_rating = ' . (int)$rating . ', story_votes = ' . (int)$votes . ' WHERE story_id = ' . (int)$id;
