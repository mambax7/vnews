...
class vnews_story extends XoopsObject {
    ...
    public function vnews_story() {
        ...
        $this->table = $this->db->prefix ( 'vnews_story' );
    }

    /**
     * Submit form in admin side
     */
    public function News_StoryForm($story_type = 'vnews') {
        $form = new XoopsThemeForm ( _VNEWS_AM_STORY_FORM, 'vnews', 'backend.php', 'post' );
        ...
        // Content title
        $form->addElement ( new XoopsFormText ( _VNEWS_AM_STORY_FORMTITLE, 'story_title', 50, 255, $this->getVar ( 'story_title', 'e' ) ), true );
        // Content alias text
        $form->addElement ( new XoopsFormText ( _VNEWS_AM_STORY_FORMALIAS, 'story_alias', 50, 255, $this->getVar ( 'story_alias', 'e' ) ), true );
        // subtitle
        $form->addElement ( new XoopsFormText ( _VNEWS_AM_STORY_SUBTITLE, 'story_subtitle', 50, 255, $this->getVar ( 'story_subtitle', 'e' ) ), false );
        // Topic
        ...
        if ($topic) {
            ...
            $topic_sel = new XoopsFormLabel ( _VNEWS_AM_STORY_TOPIC, ob_get_contents () );
            $topic_sel->setDescription ( _VNEWS_AM_STORY_TOPIC_DESC );
        ...
        if (class_exists ( 'XoopsFormEditor' )) {
            $configs = array ('name' => 'story_desc', 'value' => $this->getVar ( 'story_short', 'e' ), 'rows' => 25, 'cols' => 90, 'width' => '100%', 'height' => '400px', 'editor' => xoops_getModuleOption ( 'form_editor', 'vnews' ) );
        ...
        if (class_exists ( 'XoopsFormEditor' )) {
            $configs = array ('name' => 'story_desc', 'value' => $this->getVar ( 'story_text', 'e' ), 'rows' => 25, 'cols' => 90, 'width' => '100%', 'height' => '400px', 'editor' => xoops_getModuleOption ( 'form_editor', 'vnews' ) );
        ...
        // options
        $options = new XoopsFormElementTray ( _VNEWS_AM_STORY_OPTIONS, '<br />' );
        if (! VnewsUtils::News_UtilityEditorHTML (  )) {
            if ($this->isNew ()) {
            ...
            $html_checkbox->addOption ( 1, _VNEWS_AM_STORY_DOHTML );
            $options->addElement ( $html_checkbox );
            // Break line
            $breaks_checkbox = new XoopsFormCheckBox ( '', 'dobr', $this->getVar ( 'dobr', 'e' ) );
            $breaks_checkbox->addOption ( 1, _VNEWS_AM_STORY_BREAKS );
        ...
        $doimage_checkbox->addOption ( 1, _VNEWS_AM_STORY_DOIMAGE );
        $options->addElement ( $doimage_checkbox );
        // Xoops Code
        $xcodes_checkbox = new XoopsFormCheckBox ( '', 'doxcode', $this->getVar ( 'doxcode', 'e' ) );
        $xcodes_checkbox->addOption ( 1, _VNEWS_AM_STORY_DOXCODE );
        $options->addElement ( $xcodes_checkbox );
        // Xoops Smiley
        $smiley_checkbox = new XoopsFormCheckBox ( '', 'dosmiley', $this->getVar ( 'dosmiley', 'e' ) );
        $smiley_checkbox->addOption ( 1, _VNEWS_AM_STORY_DOSMILEY );
        $options->addElement ( $smiley_checkbox );
        // Editor and options
        $form->addElement ($options);
        // Image
        $story_img = $this->getVar ( 'story_img' ) ? $this->getVar ( 'story_img' ) : 'blank.gif';
        $uploadirectory_story_img = xoops_getModuleOption ( 'img_dir', 'vnews' ) . '/original';
        $uploadirectory_thumb_img = xoops_getModuleOption ( 'img_dir', 'vnews' ) . '/thumb/';
        $fileseltray_story_img = new XoopsFormElementTray ( _VNEWS_AM_GLOBAL_IMG, '<br />' );
        ...
        $fileseltray_story_img->addElement ( new XoopsFormFile ( _VNEWS_AM_GLOBAL_FORMUPLOAD, 'story_img', xoops_getModuleOption ( 'img_size', 'vnews' ) ), false );
        $form->addElement ( $fileseltray_story_img );
        // Files
        $uploadirectory_file = xoops_getModuleOption ( 'file_dir', 'vnews' );
        $fileseltray_file = new XoopsFormFile ( _VNEWS_AM_FILE_SELECT, 'file_name', xoops_getModuleOption ( 'file_size', 'vnews' ) );
        $file = new XoopsFormElementTray ( _VNEWS_AM_FILE );
        $file->addElement ( $fileseltray_file );
        $file->setDescription ( _VNEWS_AM_STORY_FILE_DESC );
        ...
        // Content author
        $form->addElement ( new XoopsFormText ( _VNEWS_AM_STORY_FORMAUTHOR, 'story_author', 50, 255, $this->getVar ( 'story_author', 'e' ) ), false );
        // Content Source
        $form->addElement ( new XoopsFormText ( _VNEWS_AM_STORY_FORMSOURCE, 'story_source', 50, 255, $this->getVar ( 'story_source', 'e' ) ), false );
      // publish and expire date
        $date_option = new XoopsFormElementTray(_VNEWS_AM_STORY_PEDATE,'<br />');
       $check = $this->getVar ( 'story_create' ) != $this->getVar ( 'story_publish' ) ? 1 :0;
       $publish_checkbox = new XoopsFormCheckBox('', 'autopublish', $check);
       $publish_checkbox->addOption(1, _VNEWS_AM_STORY_SETDATETIME);
       $date_option->addElement($publish_checkbox);
       $date_option->addElement(new XoopsFormDateTime(_VNEWS_AM_STORY_SETDATETIME, 'story_publish', 15,  $this->getVar ( 'story_publish' )));
       $check = $this->getVar ( 'story_expire' ) > 0 ? 1 :0;
       $expir_checkbox = new XoopsFormCheckBox('', 'autoexpire', $check);
       $expir_checkbox ->addOption(1, _VNEWS_AM_STORY_SETEXPDATETIME);
       $date_option->addElement($expir_checkbox );
       $date_option->addElement(new XoopsFormDateTime(_VNEWS_AM_STORY_SETEXPDATETIME, 'story_expire', 15,  $this->getVar ( 'story_expire' )));
      $form->addElement($date_option);
        // Active
        $form->addElement ( new XoopsFormRadioYN ( _VNEWS_AM_STORY_FORMACTIF, 'story_status', $this->getVar ( 'story_status', 'e' ) ) );
        // Default
        $form->addElement ( new XoopsFormRadioYN ( _VNEWS_AM_STORY_FORMDEFAULT, 'story_default', $this->getVar ( 'story_default', 'e' ) ) );
        // Important
        $form->addElement ( new XoopsFormRadioYN ( _VNEWS_AM_STORY_IMPORTANT, 'story_important', $this->getVar ( 'story_important', 'e' ) ) );
        // Slide
        $form->addElement ( new XoopsFormRadioYN ( _VNEWS_AM_STORY_SLIDE, 'story_slide', $this->getVar ( 'story_slide', 'e' ) ) );
      // Marque
        $form->addElement ( new XoopsFormRadioYN ( _VNEWS_AM_STORY_MARQUE, 'story_marquee', $this->getVar ( 'story_marquee', 'e' ) ) );
        // Submit buttons
     ...
     * Submit form in admin side
     */
    public function News_StorySimpleForm($story_type = 'vnews') {
        $form = new XoopsThemeForm ( _VNEWS_AM_STORY_FORM, 'vnews', 'submit.php', 'post' );
        $form->setExtra ( 'enctype="multipart/form-data"' );
        ...
        $topics = VnewsPermission::News_PermissionItemId ( 'vnews_submit');
        $criteria = new CriteriaCompo ();
        global $xoopsUser;
        if ($xoopsUser) {
 ...
 *
 **/
class VnewsStoryHandler extends XoopsPersistableObjectHandler {

    /**
     * @param $db
     */
    public function VnewsStoryHandler($db) {
        parent::XoopsPersistableObjectHandler ( $db, 'vnews_story', 'vnews_story', 'story_id', 'story_alias' );
    }
    ...
    public function News_StoryDefault($default_info) {
        ...
        $storyDefault ['imageurl'] = XOOPS_URL . xoops_getModuleOption ( 'img_dir', 'vnews' ) . '/medium/' . $storyDefault ['story_img'];
        $storyDefault ['thumburl'] = XOOPS_URL . xoops_getModuleOption ( 'img_dir', 'vnews' ) . '/thumb/' . $storyDefault ['story_img'];
        $storyDefault ['topic'] = $default_info ['title'];
        $storyDefault ['topic_alias'] = $default_info ['alias'];
        $storyDefault ['url'] = VnewsUtils::News_UtilityStoryUrl ( $storyDefault );
        ...
        if ($obj) {
            ...
            if (is_array($story_infos ['topics'])) {
                ...
                if ($root->getVar ( 'story_topic' )) {
                    ...
                    $tab ['topicurl'] = VnewsUtils::News_UtilityTopicUrl ( array('topic_id'=>$list [$root->getVar ( 'story_topic' )] ['topic_id'], 'topic_alias'=>$list [$root->getVar ( 'story_topic' )] ['topic_alias'] ));
                }
                ...
                $tab ['thumburl'] = XOOPS_URL . xoops_getModuleOption ( 'img_dir', 'vnews' ) . '/thumb/' . $root->getVar ( 'story_img' );
                $ret [] = $tab;
            }
        }
     ...
     * use in homepage public function in VnewsUtils class
     */
    ...
    public function News_StorySearch($queryArray, $condition, $limit, $start, $userId) {
        ...
        foreach ($stores as $story) {
            ...
            $data ['link'] = VnewsUtils::News_UtilityStoryUrl ( 'vnews', $data );
  ...
  /**
    ...
    */
     public function News_StoryArchiveMonth() {
         $sql = "SELECT `story_publish` FROM ".$this->db->prefix('vnews_story')." WHERE ( story_status = 1 ) AND ( story_topic != 0  ) AND ( story_publish > 0 AND story_publish <= " . time() . " ) AND ( story_expire = 0 OR story_expire <= " . time() . " ) ORDER BY story_publish DESC";
        $result = $this->db->query($sql);
  ...
  /**
    ...
    */
    ...
    {
        $sql = 'UPDATE ' . $this->db->prefix('vnews_story') . ' SET story_rating = ' . intval($rating) . ', story_votes = ' . intval($votes) . ' WHERE story_id = ' . intval($id);
